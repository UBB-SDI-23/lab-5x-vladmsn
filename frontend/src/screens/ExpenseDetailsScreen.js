...
const GroupDetailScreen = ({ match }) => {
  ...
  useEffect(() => {
    const fetchGroupDetails = async () => {
      try {
       ...
       (response.data.participants.find(participant => participant.id === userInfo.userId)) ? setIsParticipant(true) : setIsParticipant(false);
  ...
  const addParticipant = async () => {
    try {
      const response = await axios.post(`/api/v1/groups/participant?groupId=${id}&userId=${userInfo.userId}`);
  ...
  const removeParticipant = async () => {
    try {
      const response = await axios.delete(`/api/v1/groups/participant?groupId=${id}&userId=${userInfo.userId}`);
    ...
    <Container>
      <Row className="my-3">
        <Col sm={12} md={8}>
          ...
          {userInfo.userId === group.ownerId && 
            (<Button className="my-3">Delete</Button>)}

          {userInfo.role === "MODERATOR_ROLE" &&
